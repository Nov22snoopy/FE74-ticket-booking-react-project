...
export const movieBookingReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case PURCHASE:{
      const chairBooking = state.ChairBookingList.map((e)=>e.soGhe)
      const chairList = state.ChairList.map((item)=>{
        const chairNew = item.danhSachGhe.map((chair)=>{
          if(chairBooking.includes(chair.soGhe)){
            chair.daDat = true
          }
          return chair
        })
        return {
          ...item,
          danhSachGhe: chairNew
        }
      })
      return{...state, chairList: chairList, ChairBookingList: []}
    }
    case DELETE:{
      console.log(action.payload);
      const chairBooking = [...state.ChairBookingList];
      const index = chairBooking.findIndex((item) => item.soGhe === action.payload);
      if(index !== -1) {
        chairBooking.splice(index, 1);
      };
      return {...state, ChairBookingList:chairBooking}
    }
    default:
      return state;
  }
